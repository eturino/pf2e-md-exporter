...
const DOCUMENT_ICON = {
  ...
  table: {
    Actor: ":RaKnightHelmet:",
    Cards: ":RaSpadesCard:",
    ChatMessage: ":RaSpeechBubble:",
    Combat: ":RaCrossedSwords:",
    Item: ":LiLuggage:",
    Folder: ":LiFolder:",
    JournalEntry: ":RaBook:",
    JournalEntryPage: ":RaScrollUnfurled:",
    Playlist: ":LiListMusic:",
    RollTable: ":LiTable:",
    Scene: ":LiMap:",
  },
  ...
  //User: ""
  lookup(document) {
    return this.table[document.documentName] || ":LiFileQuestion:";
  },
};
...
function templateFile(doc) {
  ...
  const base = doc instanceof Actor ? "Actor" : doc instanceof Item ? "Item" : undefined;
  if (!base) return undefined;
  return (
    game.settings.get(MODULE_NAME, `template.${base}.${doc.type}`) || game.settings.get(MODULE_NAME, `template.${base}`)
  );
}

function templateForMEJContent() {
  return game.settings.get(MODULE_NAME, "template.MEJPage");
}
function templateForMEJFrontmatter() {
  return game.settings.get(MODULE_NAME, "template.MEJPageFrontmatter");
}
...
function fileconvert(givenFilename, label_or_size = null, inline = true) {
  const filename = decodeURIComponent(givenFilename);
  ...
  const basefilename = treatImageFilename(filename);

  zip.folder(destForImages).file(
    basefilename,
...
function expandOneLocalize(str, type, target, _hash, _label, _offset, _string, _groups) {
  if (type === "Localize") {
...
function makeReplaceOneLinkForConvertLinks(relativeTo) {
  return function replaceOneLink(str, givenType, givenTarget, hash, givenLabel, _offset, _string, _groups) {
    let type = givenType;
    ...
    const filename = notefilename(linkDoc);
    ...
    if (linkDoc) {
      ...
      const pack = game.packs.get(linkDoc.pack);
      if (pack) {
        // Slashes in the title aren't real paths and as part of the export become underscores
        const fixedTitle = pack.title.replaceAll("/", "_");
        result = `${fixedTitle}/${result}`;
      }
    }
...
function makeMarkdownLinkForUUIDDoc(relativeTo) {
  const replaceOneLink = makeReplaceOneLinkForConvertLinks(relativeTo);
  return function markdownLinkForUUIDDoc(uuid, label) {
    return replaceOneLink(`@UUID[${uuid}]{${label}}`, "UUID", uuid, null, label);
  };
}

function convertLinks(givenMarkdown, relativeTo) {
  const replaceOneLink = makeReplaceOneLinkForConvertLinks(relativeTo);
  let markdown = givenMarkdown;
...
function makeReplaceOneLinkForMarkdownLinks(relativeTo) {
  return function replaceOneLink(_str, givenTarget, givenLabel) {
    let target = givenTarget;
    let label = givenLabel;
    function dummyLink() {
      // Make sure that "|" in the ID don't start the label early (e.g. @PDF[whatever|page=name]{label})
...
function convertMarkdownLinks(markdown, relativeTo) {
  ...
  const replaceOneLink = makeReplaceOneLinkForMarkdownLinks(relativeTo);

  // Convert all the links with UUIDs to human readable links
  // Look for [[link|label]]
  const pattern = /\[\[([^\]\|]+)\|([^\]]+)\]\]/g;
  return markdown.replace(pattern, replaceOneLink);
}
 ...
 */
function doMath(doc, givenFormula) {
  ...
  if (!formula.includes("d") && !formula.includes("D")) {
    ...
    const rollTerms = Roll.parse(formula);
...
export function convertHtml(doc, html) {
  ...
  if (!turndownService) {
    ...
    turndownService.addRule("actionGlyph", {
      filter: (node, options) => node.nodeName === "SPAN" && node.getAttribute("class") === "action-glyph",
      replacement: (givenContent, node, options) => {
  ...
  try {
    ...
    markdown = markdown.replace(roll2Pattern, (match, p1) => `${p1}`);
    ...
    markdown = markdown.replace(rollPattern, (match, p1, p2) => {
      const result = doMath(doc, p1);
    ...
    markdown = markdown.replace(genericPattern, (match, p1, p2) => {
    ...
    markdown = markdown.replace(damagePattern, (match, p1, givenP2) => {
      let p2 = givenP2;
    ...
    markdown = markdown.replace(damage2Pattern, (match, p1) => {
      let result = doMath(doc, p1);
      if (result) {
        // Remove any wrapping parens
        result = `${result}`.replace(/^\(|\)$/g, "");
    ...
    markdown = markdown.replace(templatePattern, (match, p1, p2) => `${p2}-foot ${p1}`);
    ...
    markdown = markdown.replace(checkPattern, (match, givenP1, p2) => {
      let p1 = givenP1;
    ...
    markdown = markdown.replace(checkBasicPattern, (match, givenP1, basic) => {
      let p1 = givenP1;
      // Convert sluggified p1 to more friendly label
      p1 = p1
        .split("-")
    ...
    markdown = turndownService
      .turndown(convertLinks(markdown, doc))
      .replaceAll("\\[\\[", "[[")
      .replaceAll("\\]\\]", "]]");
...
function frontmatter(doc, opts = {}) {
  const extraFrontmatter = opts.extraFrontmatter || "";
  const header = opts.showHeader ? `\n# ${doc.name}\n` : "";

  return (
    // biome-ignore lint/style/useTemplate: in this case it's easier to read the code without a template
    FRONTMATTER +
    `title: "${doc.name}"\n` +
...
async function oneJournal(path, journal) {
  let subpath = path;
  const extraFromJournal = await extraForMonksEnhancedJournal(journal);
  ...
  for (const page of journal.pages) {
    let markdown;
    const extraForMEJ = await extraForMonksEnhancedJournalPage(page, extraFromJournal);
    switch (page.type) {
      case "text":
    ...
    markdown = extraForMEJ.markdown ? [markdown, extraForMEJ.markdown].join(EOL) : markdown;
    if (markdown) {
      markdown =
        frontmatter(page, { extraFrontmatter: extraForMEJ.frontmatter, showHeader: page.title.show }) + markdown;
      zip.folder(subpath).file(`${notefilename(page)}.md`, markdown, { binary: false });
    }
  }
}

async function extraForMonksEnhancedJournal(journal) {
  const owner = getOwner(journal);
  const mej = getProperty(journal, "flags.monks-enhanced-journal");
  if (!mej) {
    return { owner: owner, mej: null, img: null };
  }
  return { owner: owner, mej: mej, img: mej.img ? treatDocImage(mej.img) : null };
}

function getOwner(doc) {
  if (!doc.ownership) return null;
  const level = doc.ownership[game.userId] ?? doc.ownership.default;
  if (level > 0) {
    return game.userId;
  }
  return null;
...
async function extraForMonksEnhancedJournalPage(page, extraFromJournal) {
  const owner = getOwner(page) ?? extraFromJournal.owner;
  const mej = getProperty(page, "flags.monks-enhanced-journal");
  if (!mej) {
    return { frontmatter: "", markdown: "" };
  }

  const rawNotes = mej[owner] ? mej[owner].notes : null;
  const notes = rawNotes ? convertHtml(page, rawNotes) : null;

  const pageImg = mej.img ? treatDocImage(mej.img) : extraFromJournal.img;
  const actor = actorFromMEJPage(mej, pageImg, page);
  const attributesTableData = attributesTableDataFromMEJPage(mej, actor);
  const mejData = {
    owner: owner,
    page: page,
    mej: mej,
    mejJournal: extraFromJournal.mej,
    extraFromJournal: extraFromJournal,
    rawNotes: rawNotes,
    notes: notes,
    img: pageImg,
    actor: actor,
    relationships: relationshipsFromMEJPage(mej, page),
    relationshipsTableData: relationshipsTableDataFromMEJPage(mej, page),
    attributesTableData: attributesTableData,
    location: mej.location,
    role: mej.role,
  };
  let markdown = "";
  let frontmatter = "";
  const templatePathForFrontmatter = templateForMEJFrontmatter();
  const templatePathForContent = templateForMEJContent();
  if (templatePathForFrontmatter) {
    frontmatter = await markdownFromDocWithTemplate(templatePathForFrontmatter, mejData);
  }
  if (templatePathForContent) {
    markdown = await markdownFromDocWithTemplate(templatePathForContent, mejData);
  }
  return { markdown, frontmatter };
}

function actorFromMEJPage(mej, pageImg, page) {
  if (!mej.actor) return null;

  const linkFor = makeMarkdownLinkForUUIDDoc(page);
  const actorImg = mej.actor.img ? treatDocImage(mej.actor.img) : null;
  return {
    link: linkFor(mej.actor.uuid, mej.actor.name),
    name: mej.actor.name,
    uuid: mej.actor.uuid,
    img: actorImg,
    imgIfDifferent: actorImg && actorImg !== pageImg ? actorImg : null,
    quantity: mej.actor.quantity,
  };
}

function relationshipsFromMEJPage(mej, page) {
  if (!mej.relationships) return null;

  const linkFor = makeMarkdownLinkForUUIDDoc(page);
  return mej.relationships.map((rel) => {
    const link = linkFor(rel.uuid, rel.name);
    return {
      id: rel.id,
      link: link,
      name: rel.name,
      hidden: rel.hidden,
      img: rel.img ? treatDocImage(rel.img) : null,
      relationship: rel.relationship ? convertLinks(rel.relationship, page) : null,
      role: rel.role,
      type: rel.type,
      uuid: rel.uuid,
    };
...
function relationshipsTableDataFromMEJPage(mej, page) {
  if (!mej.relationships?.length) return null;

  const linkFor = makeMarkdownLinkForUUIDDoc(page);
  const relationships = mej.relationships.map((rel) => {
    const link = linkFor(rel.uuid, rel.name);
    return {
      id: rel.id,
      link: link,
  ...
  const tableData = [["Portrait", "Person", "Relationship"]];
  for (const rel of relationships) {
    const portrait = rel.img ? `![[${rel.img}]]` : null;
    tableData.push([portrait, rel.link, rel.relationship]);
  }
  return tableData;
}

function attributesTableDataFromMEJPage(mej, actor) {
  const attributes = mej.attributes;
  const location = mej.location;
  const role = mej.role;
  if (!attributes && !location && !role) {
    return null;
  }

  const tableData = [["Attribute", "Value"]];
  if (role) {
    tableData.push(["role", role]);
  }
  if (location) {
    tableData.push(["location", location]);
  }

  for (const key in mej.attributes) {
    if (Object.hasOwn(mej.attributes, key)) {
      const value = mej.attributes[key];
      if (`${value}`.length > 0) {
...
async function oneRollTable(path, table) {
  ...
  markdown += `| ${table.formula || "Roll"} | result |\n|------|--------|\n`;

  for (const tableresult of table.results) {
    const range =
      tableresult.range[0] === tableresult.range[1]
        ? tableresult.range[0]
        : `${tableresult.range[0]}-${tableresult.range[1]}`;
    // Escape the "|" in any links
    markdown += `| ${range} | ${convertHtml(table, tableresult.getChatText()).replaceAll("|", "\\|")} |\n`;
  }
...
function oneScene(path, scene) {
  ...
  function coord2(pixely, pixelx) {
    return `${coord(sceneBottom - pixely)}, ${coord(pixelx - sceneLeft)}`;
  }
  ...
  for (const tile of scene.tiles) {
    ...
    overlays.push(
      `${fileconvert(tile.texture.src, name, /*inline*/ false)}, [${coord2(
        tile.y + tile.height - 1,
        tile.x,
      )}], [${coord2(tile.y, tile.x + tile.width - 1)}]`,
    );
  }

  const layers =
    overlays.length === 0 ? "" : `imageOverlay:\n${overlays.map((layer) => `    - [ ${layer} ]\n`).join("")}`;
  ...
  markdown +=
    ...
    `\n${MARKER}leaflet\n` +
    `id: ${scene.uuid}\n` +
    `bounds:\n    - [0, 0]\n    - [${coord(scene.dimensions.sceneRect.height)}, ${coord(
      scene.dimensions.sceneRect.width,
    )}]\n` +
    "defaultZoom: 2\n" +
...
async function documentToJSON(path, doc) {
  ...
  for (const field of DESCRIPTIONS) {
    const text = foundry.utils.getProperty(doc, field);
    if (text) markdown += convertHtml(doc, text) + EOL + EOL;
  }

  let datastring;
  const dumpoption = game.settings.get(MOD_CONFIG.MODULE_NAME, MOD_CONFIG.OPTION_DUMP);
  if (dumpoption === "YAML") datastring = jsyaml.dump(data);
  else if (dumpoption === "JSON") datastring = JSON.stringify(data, null, 2) + EOL;
  else console.error(`Unknown option for dumping objects: ${dumpoption}`);
  ...
  markdown += MARKER + doc.documentName + EOL + datastring + MARKER + EOL;
...
async function maybeTemplate(path, doc) {
  ...
  doc.img = doc.img ? treatDocImage(doc.img) : null;
  ...
  const markdown = await markdownFromDocWithTemplate(templatePath, doc);

  zip.folder(path).file(zipfilename(doc), markdown, { binary: false });
}

async function markdownFromDocWithTemplate(templatePath, doc) {
  // Apply the supplied template file:
  // Foundry renderTemplate only supports templates with file extensions: html, handlebars, hbs
  // Foundry filePicker hides all files with extension html, handlebars, hbs
...
function treatDocImage(img) {
  if (!img) return null;

  fileconvert(img, IMG_SIZE);
  ...
  return treatImageFilename(img);
}

function treatImageFilename(imgFilename) {
  return imgFilename.replaceAll("/", "-").slice(-250 + destForImages.length);
}

async function oneDocument(path, doc) {
  if (doc instanceof JournalEntry) await oneJournal(path, doc);
  else if (doc instanceof RollTable) await oneRollTable(path, doc);
  else if (doc instanceof Scene && game.settings.get(MOD_CONFIG.MODULE_NAME, MOD_CONFIG.OPTION_LEAFLET))
    oneScene(path, doc);
  else if (doc instanceof Playlist) onePlaylist(path, doc);
  else await maybeTemplate(path, doc);
  // Actor
  // Cards
  // ChatMessage
  // Combat(Tracker)
...
async function oneFolder(path, folder) {
  const subpath = formpath(path, validFilename(folder.name));
...
async function onePack(path, pack) {
  ...
  const subpath = formpath(path, validFilename(pack.title));
...
export async function exportMarkdown(from, zipname) {
  ...
  const noteid = ui.notifications.info(`${MODULE_NAME}.ProgressNotification`, { permanent: true, localize: true });
  ...
  if (from instanceof Folder) {
    ...
    if (from.type === "Compendium") await onePackFolder(TOP_PATH, from);
    else await oneFolder(TOP_PATH, from);
  ...
  } else await oneDocument(TOP_PATH, from);
...
Hooks.once("init", async () => {
  ...
  libWrapper.register(
    MODULE_NAME,
    `${baseClass}.prototype._getFolderContextOptions`,
    addFolderMenu,
    libWrapper.WRAPPER,
  );
  if (!is_v10)
    libWrapper.register(
      MODULE_NAME,
      "CompendiumDirectory.prototype._getFolderContextOptions",
      addFolderMenu,
      libWrapper.WRAPPER,
    );
});
...
Hooks.on("renderSidebarTab", async (app, html) => {
  ...
  if (!(app instanceof Settings)) {
    ...
    const button = $(`<button style="flex: 0" title="${help}"><i class='fas fa-file-zip'></i>${label}</button>`);
    button.click((event) => {
