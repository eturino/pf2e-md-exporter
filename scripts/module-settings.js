...
Hooks.once("ready", () => {
  ...
  Handlebars.registerHelper("me-trait", (value) => {
    // Convert a sluggified trait into its localized human-readable text
    const lookUpText = CONFIG.PF2E.npcAttackTraits[value];
    if (lookUpText) {
  ...
  Handlebars.registerHelper("me-spellLevels", (items, id, spellType) => {
    ...
    if (items) {
      for (item of items) {
        if (item.type === "spell" && item.system.location.value === id) {
          ...
          const includesConstant = item.name.includes("(Constant)");

          if (
            (spellType === "cantrips" && item.isCantrip) ||
            (spellType === "constant" && includesConstant) ||
            (spellType === "spells" && !includesConstant)
          ) {
  ...
  Handlebars.registerHelper("me-getSpellSlotCount", (item, level) => {
    // Return the the spell slot count for the given level in the
    // form "(# slots) if it is non-zero.

    const slots = item.system.slots[`slot${level}`];
    if (slots && slots.value !== 0) {
      return `(${String(slots.value)} slots)`;
    }
    return "";
  });

  Handlebars.registerHelper("me-equipmentList", (items) => {
    // Return the list of items with the system.equipped property present.
    const equipmentList = [];
    for (const item of items) {
      if (item.system.equipped) {
        const quantity = item.system.quantity ? item.system.quantity : 1;
  ...
  Handlebars.registerHelper("me-getRituals", (items) => {
  ...
  Handlebars.registerHelper("me-getSpellList", (items, id, level, spellType) => {
    ...
    if (items) {
      for (const item of items) {
        if (item.type === "spell" && item.system.location.value === id) {
          const spellLevel = item.system.location.heightenedLevel || item.level || item.system.level.value;
          if (level === spellLevel) {
            const includesConstant = item.name.includes("(Constant)");
            if (
              (spellType === "cantrips" && item.isCantrip) ||
  ...
  Handlebars.registerHelper("me-HTMLtoYAML", (givenText, context, options) => {
    if (!givenText) return givenText;
  ...
  Handlebars.registerHelper("me-HTMLtoMarkdown", (text, context) => {
    if (text) {
      return convertHtml(context, text);
    }
    return text;
  });

  Handlebars.registerHelper("me-debugDumpObject", (msg, options) => {
...
Hooks.on("renderSettingsConfig", (app, html, options) => {
  ...
  actorModuleTab
    ...
    .before(`<h2 class="setting-header">${game.i18n.localize(`${MOD_CONFIG.MODULE_NAME}.titleActorTemplates`)}</h2>`);
  ...
  itemModuleTab
    ...
    .closest("div.form-group")
    .before(`<h2 class="setting-header">${game.i18n.localize(`${MOD_CONFIG.MODULE_NAME}.titleItemTemplates`)}</h2>`);
});
